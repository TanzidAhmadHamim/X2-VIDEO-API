...
try {
    var i, j, n = 0;
    let srcPath = path.join(process.cwd(), "/x2files/");
    ...
    for (var dir of getDirs) {
        fileName = readdirSync(path.join(process.cwd(), '/x2files/' + dir + '/')).filter((file) => file.endsWith(".js") && file !== 'x2.js');
        for (j of fileName) {
            var { index, name } = require(path.join(process.cwd(), '/x2files/' + dir + '/') + j);
            router.get(name, index);
            n++
        }
    }
